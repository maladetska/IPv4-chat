...
namespace chat {
    ...
    int SocketWorker::Socket() {
        return CheckError("Socket failed", socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP));
    }

    void SocketWorker::Bind() {
        CheckError("Bind failed",
                   bind(sockfd_, reinterpret_cast<const sockaddr *>(&addr_out_),
                        sizeof(addr_out_)));
    }

    void SocketWorker::Setsockopt() {
        CheckError("Setsockopt failed", setsockopt(sockfd_, SOL_SOCKET, SO_BROADCAST,
                                                   &kBroadcast, sizeof(kBroadcast)));
    }
    ...
    void SocketWorker::Sendto() {
        ...
        CheckError(
                ...
                static_cast<int>(sendto(
                        sockfd_, text_buffer_out_.GetText().c_str(),
                        text_buffer_out_.GetText().size(), 0,
                        reinterpret_cast<const sockaddr *>(&broadcast_addr_), sockaddr_len)));
    }
    ...
    void SocketWorker::Recvfrom() {
        ...
        CheckError("Recvfrom failed",
                   static_cast<int>(recvfrom(
                           sockfd_, text_buffer_in_.GetText().data(),
                           text_buffer_in_.GetMaxSize(), MSG_WAITALL,
                           reinterpret_cast<sockaddr *>(&addr_in_), &sockaddr_len)));
    }
    ...
    void SocketWorker::CloseSocket() const { close(sockfd_); }
    ...
    void SocketWorker::FillAddressInfo(sockaddr_in &addr, in_addr_t host,
                                       in_port_t port) {
    ...
    std::string &SocketWorker::TextBuffer::GetText() { return text_; }

    size_t SocketWorker::TextBuffer::GetMaxSize() const { return max_size_; }
